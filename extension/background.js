...
async function launch(windowId) {
  ...
  // Static and settings are appended once here, after the real channels —
  // any empty dial slot switches to the shared static tab rather than
  // needing one of its own, and settings is just another tab in the
  // rotation (the vanity "U" slot) instead of a one-off tab created and torn
  // down on every visit. Loading them last means the power-on flicker
  // touches the real channel tabs before it lands on those two.
